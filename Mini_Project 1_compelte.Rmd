...
ggplot(average_lifeExpectancy_continent_year, aes(x = year, y = averageLifeExpectancy, group = continent, color = continent)) + geom_line()+ scale_x_discrete(breaks = seq(1950, 2020, by = 10)) + ggtitle("Average life expectancy changes by years in each continent \nsince World War 2 ") + labs(x = "year", y="average life expectancy")
```
...
ggplot(dataset6, aes(x = averageGDPpercapita, y = averageLifeExpectancy, group = continent, color = continent)) + geom_point() + ggtitle("life expectancy versus GDP per capital in each continent since \nWorld War 2") + labs(x = "GDP per capital", y = "average life expectancy(weighted)")
